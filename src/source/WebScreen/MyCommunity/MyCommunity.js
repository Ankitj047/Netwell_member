...
export default class Transaction extends Component {
  ...
  updateDraftDay = draftDaySelected => {
    this.setState({
      errorModal: true,
      open: false,
      draftDayModal: false,
    ...
    // updateDraftDayData(draftDaySelected).then(res => {
    //   if (res && res.data) {
    //     if (res.data.code === 200) {
    //       this.setState({
    //         errorModal: true,
    ...
    //         draftDayModal: false,
    //         loader: false,
    //         MsgModal: 'Update recurring date and draft date successfully!'
    //       })
    //       this.getPaymentData()
    //     } else {
    //       this.setState({ errorModal: true, open: false, draftDayModal: false, loader: false, MsgModal: 'Member not found!' })
    //     }
    //   }
    // })
  }
        ...
        <div className="container progInfoContainer" style={{ zIndex: '0' }}>
          ...
          <div className="commonWrap" style={{ marginTop: '-27px' }}>
            <div className="progInfoSection" style={{ marginTop: '-15px' }}>
              <span className="progInfoTitle">MY CONTRIBUTIONS</span>
            </div>
            ...
            <div className="tablebackgroundouter">
              <div style={{ display: 'flex', flexDirection: 'row' }}>
                ...
                <div>
                  Your monthly contributions are voluntary contributions made to help other Universal Healthshare member with their eligible
                  healthcard expenses. Each month you will be assigned one or more specific medical exepnse needs of another member or
                  members in which to share. here are the needs of other members that have been met by your contributions.
                </div>
              </div>
            </div>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={12} style={{ marginBottom: 10 }}>
                  ...
                  <p class="The-fellowship-incurs-additional-fees-related-to-p" style={{ marginLeft: '5px' }}>
                    To make it easier for you to plan your cash flow, you can decide when your monthly payment is processed. Please select
                    how many days before payment due date, you would like your account to be charged.
                  </p>
                </Grid>
              </Grid>
                  ...
                  {/* <div>
                    <FormControl component="fieldset">
                      <RadioGroup
                        ...
                        onChange={event => this.answerChangeHandler(event, 'radio', '')}
                      >
                        <FormControlLabel value="5" control={<PurpleRadio />} label="5 days" />
                        <FormControlLabel value="7" control={<PurpleRadio />} label="7 days" />
                        <FormControlLabel value="10" control={<PurpleRadio />} label="10 days" />
                        <FormControlLabel value="15" control={<PurpleRadio />} label="15 days" />
                      </RadioGroup>
                    </FormControl>
                  </div> */}
                </Grid>
                    ...
                    <div className="paymentDatesWrapper">
                      ...
                      <div style={{ marginLeft: '5px' }}>
                        {this.state.paymentType === 'Credit Card' ? (
                          <h6>
                            <span>&#42;</span>3.5% merchant fees apply on credit card payments
                          </h6>
                        ) : null}
                        <h6>
                          <span>
                          ...
                          The actual date may change based on the number of days in the month
                        </h6>
                      </div>
                    </div>
                  </div>
          ...
          <Modal.Footer>
            {localStorage.getItem('SOURCE') === 'NEO' ? (
              ...
              <>
                ...
                <NextButton
                  variant="contained"
                  disabled={this.state.draftDaySelected == '' || this.state.draftDaySelected == null}
                  color="primary"
                  onClick={() => this.updateDraftDay(this.state.draftDaySelected)}
                >
                  PROCEED
                </NextButton>
              </>
    ...
    return this.state.loader ? null : (
      <div class="paymentBlock">
        ...
        <div style={{ marginTop: '20px' }}>
          ...
          <hr />
          <div>
            <span class="TitleCurrentPaymentMethod">Recurring Monthly Payments</span>
            <br />
            <span class="numberPaymentMethod">{this.getRecurringDay()}</span>
            <div>
              <button
                type="button"
                onClick={this.openDraftDayModal}
